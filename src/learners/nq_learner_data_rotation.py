...
from utils.data_processing import data_random_rotate, data_symmetrize
...
class NQLearnerDataRotation:
    ...
    def train_each_batch(self, batch):
        ...
        # inverse the output Q as no rotation
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        th.save(batch,"test_data/batch_data.pt")
        start_time = time.time()
        if self.args.use_cuda and str(self.mac.get_device()) == "cpu":
            self.mac.cuda()

        # Do data augmentation here.
        # for data_rotation_time in range(1, 4):
        #     bs = batch.batch_size
        #     theta = 0.5 * th.pi * th.ones(bs) * data_rotation_time
        #     new_batch = data_random_rotate(self.args, batch, theta)
        #     loss, td_error_abs, q_taken_mean, target_mean, grad_norm = self.train_each_batch(new_batch)
        #     del new_batch

        for data_symm_axis in range(2):
            new_batch = data_symmetrize(self.args, batch, data_symm_axis)
            loss, td_error_abs, q_taken_mean, target_mean, grad_norm = self.train_each_batch(new_batch)
            del new_batch
