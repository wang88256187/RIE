...
def run(_run, _config, _log):
    ...
    testing_algorithms = ["vdn", "qmix", "hpn_vdn", "hpn_qmix",
                          ...
                          "gnn_vdn", "gnn_qmix", "qplex", "hpn_qplex", "asn", "ir_qmix"
                          ]
    env_name = args.env
    logdir = env_name
    if env_name in ["sc2"]:
    ...
    if env_name in ["sc2wrapped"]:
        logdir = os.path.join("{}_{}-nagents_{}_nenemies_{}_obs_aid={}-obs_act={}".format(
            logdir,
            args.env_args["map_name"],
            int(args.env_args["capability_config"]["n_units"]),
            int(args.env_args["capability_config"]["n_enemies"]),
            int(args.obs_agent_id),
            int(args.obs_last_action),
        ))
    ...
    if args.name in testing_algorithms:
        ...
        elif args.name in ["hpn_vdn", "hpn_qmix", "hpn_qplex", "ir_qmix"]:
            logdir = os.path.join(logdir,
...
def evaluate_sequential(args, runner):
    ...
    for _ in range(args.test_nepisode):
        ...
        batchs.append(batch)
    win_rate = runner.logger.stats['test_battle_won_mean'][0][1]
    print("Winning rate: {}".format(win_rate))
    filename =  "data_for_evaluate/" + args.env_args["map_name"] + "_" + "batchs.th"
    torch.save(batchs, filename)
    if args.save_replay:
        runner.save_replay()
...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        if buffer.can_sample(args.batch_size):   # buffer里的数据大于batch size,则采样
